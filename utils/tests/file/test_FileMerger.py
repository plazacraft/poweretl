from poweretl.utils.file.FileMerger import FileMerger
from poweretl.utils.file.MultiFileReader import MultiFileReader, FileEntry
...
def test_merge_files():
    ...
    tokens_replacer = TokensReplacer(re_start=r"(/\*<)|(<)", re_end=r"(>\*/)|(>)", re_escape=r"\^")

    merger = FileMerger()
    config_reader_1 = MultiFileReader(
            [FileEntry(f'{data_dir}/config', r'\.jsonc?$')]
        )
    config_files_1 = config_reader_1.get_files_with_content()

    param_reader_1 = MultiFileReader(
            [FileEntry(f'{data_dir}/config', r'\.yaml?$')]
        )
    param_files_1 = param_reader_1.get_files_with_content()

    params_1 = merger.merge(param_files_1)
    config_contents_1 = [(config, tokens_replacer.replace(tokens=params_1, text=content)) for config, content in config_files_1]
    
    config_1 = merger.merge(config_contents_1)



    config_reader_2 = MultiFileReader(
            [FileEntry(f'{data_dir}/result', r'\.json?$')]
        )
...
def test_merge_files_empty():
    ...
    assert merger.merge(config_files_1) == None, "Result is not empty"
...
def test_merge_files_unsupported():
    ...
    merger = FileMerger()
    config_reader_1 = MultiFileReader(
            [FileEntry(f'{data_dir}/config', r'\.txt$')]
        )
    config_files_1 = config_reader_1.get_files_with_content()

...
class DbxModelManager(BaseModelManager):
    ...
    def get_table_model_from_source(self, table_name: str) -> Table:
        """Get table model from Databricks source by querying the actual table metadata."""
        try:
            import json
            import re
            ...
            for row in describe_rows:
                ...
                col_comment = row['comment'] if 'comment' in row.asDict() and row['comment'] else None
                ...
                if not in_detailed_info and not in_clustering_info and col_name and data_type:
                    column = Column(
                        name=col_name,
                        type=data_type,
                        comment=col_comment,
                ...
                # Parse detailed table information
                    ...
                    if col_name == "Location":
                        ...
                        # Table Properties section exists but we'll read properties from SHOW TBLPROPERTIES instead
                        pass
            ...
            for row in tblproperties_df.collect():
                ...
                if key:
                    ...
                    if key == "clusteringColumns":
                        # Parse JSON array format [["col1"],["col2"]] and convert to (col1, col2)
                        import json
                        # Remove escaped quotes if present
                        value_clean = value.replace('""', '"')
                        cols_array = json.loads(value_clean)
                        col_names = [col[0] if isinstance(col, list) else col for col in cols_array]
            ...
            if len(table_parts) == 3:
                ...
                raise ValueError(f"Table name must be in format 'catalog.schema.table', got: {table_name}")
            
            column_tags_df = self._spark.sql(f"""
            ...
            for row in column_tags_df.collect():
                column_name = row['column_name'] if 'column_name' in row.asDict() else None
            ...
            # If any error occurs (table doesn't exist, permission issues, etc.), return None
            return None

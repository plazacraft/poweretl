...
class DbMetaProvider(BaseMetaProvider):
    ...
    def __init__(
        ...
        *,
        ...
        schema_paths: list[FileEntry] = [
            FileEntry(
                Path(os.path.dirname(os.path.abspath(__file__))).joinpath("_data").joinpath("db_meta_provider").joinpath("schema"),
                r"\.sql",
            )
        ],
        commands_paths: list[FileEntry] = [
            FileEntry(
                Path(os.path.dirname(os.path.abspath(__file__))).joinpath("_data").joinpath("db_meta_provider"),
                r"commands\.yaml",
            )
        ], 
        encoding: str = "utf-8",
        ...
        self._schema_contents = [
            (command_file, self._tokens_replacer.replace(tokens=self._command_params, text=content))
            for command_file, content in schema_files
        ]

        self._schema = self._commands_splitter.get_commands(
            self._schema_contents
        )
        ...
        self._commands = self._file_merger.merge(
            command_files
        )
    ...
    def _update_version(self, version: str, step: int):
        command = self._prepare_command("update_version", {"version": version, "step": str(step)})
